...
def create_dataloader(config):
    dataset_train = JsonDataset(config.train_file, config.train_data_dir, config=config)
    print("distributed debug message, RANK: {} and WORLD_SIZE: {}".format(get_rank(), get_world_size()))
    sampler = torch.utils.data.DistributedSampler(dataset_train, shuffle=True, num_replicas=get_world_size(),
                                                  rank=get_rank()) if config.distributed else None
