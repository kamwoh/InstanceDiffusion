...
def get_args_parser():
    ...
    parser.add_argument("--name", type=str, default="checkpoint-01",
    ...
    parser.add_argument("--yaml_file", type=str, default="configs/train_text_all_sd_v1_5.yaml",
    ...
    parser.add_argument("--train_data_dir", type=str, default="data/", help="Train data directory")

    # count number of duplicated classes when making a sentence.
    parser.add_argument("--count_dup", type=lambda x: x.lower() == "true", default=False,
                        help="count number of duplicated classes")

    # count number of duplicated classes when making a sentence.
    parser.add_argument("--re_init_opt", type=lambda x: x.lower() == "true", default=False,
                        help="reinitialize optimizer and scheduler")
    ...
    parser.add_argument("--use_masked_att", type=lambda x: x.lower() == "true", default=False,
                        help="use masked attention given the bounding box or not")

    # more options
    parser.add_argument("--add_inst_cap_2_global", type=lambda x: x.lower() == "true", default=False,
                        help="add instance captions to the global captions or not")
    parser.add_argument("--use_instance_sampler", type=lambda x: x.lower() == "true", default=False,
                        help="using multi-instance sampler during training or not")
    parser.add_argument("--mis_ratio", type=float, default=0,
                        help="the percentage of timesteps using multi-instance-sampler")
    parser.add_argument("--use_crop_paste", type=lambda x: x.lower() == "true", default=False,
                        help="using use_crop_paste for multi-instance sampler or not")
